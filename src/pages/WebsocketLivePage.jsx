...
export default function WebsocketLivePage() {
        ...
        try {
          ...
          if (data.type === "status") {
            ...
            setStreamedJobs((prev) => {
              // Prefer Mongo _id / job_url de-dupe so re-streams don't duplicate cards
  ...
  useEffect(() => {
    // Always load latest scraper config from MongoDB when opening Live Console
  ...
  return (
    <section className="page-shell">
      <div className="page-header">
        ...
        <div className="flex flex-wrap items-center gap-3">
          <button
            ...
            className={`${
              showScraperConfig
                ? "btn-primary"
                : "btn-ghost"
            } text-xs font-bold inline-flex items-center gap-1.5 rounded-xl cursor-pointer`}
          ...
          >
            ...
            {showScraperConfig ? "Hide Config Editor" : "Scraper Configuration"}
          </button>
          ...
          <span
            className={`inline-flex items-center gap-1.5 rounded-full border px-3 py-1.5 text-[11px] font-bold tracking-wide ${
              wsStatus === "connected"
                ? "border-emerald-250 bg-emerald-50/50 text-emerald-800 dark:border-emerald-950/40 dark:bg-emerald-950/40 dark:text-emerald-400"
                : wsStatus === "connecting"
                  ? "border-amber-250 bg-amber-50/50 text-amber-800 dark:border-amber-950/40 dark:bg-amber-950/40 dark:text-amber-400"
                  : "border-slate-200 bg-slate-100/50 text-slate-655 dark:border-slate-800 dark:bg-slate-900/60 dark:text-slate-400"
            }`}
          >
            <span
          ...
          {wsStatus === "disconnected" && (
            <button
              type="button"
              className="btn-ghost text-xs font-bold inline-flex items-center gap-1 py-1.5 px-3 rounded-lg cursor-pointer animate-pulse"
              onClick={() => connectWS()}
            >
              Reconnect
            </button>
      ...
      {/* Main Console Layout */}
      <div className="grid min-h-0 flex-1 grid-cols-1 gap-5 xl:grid-cols-12 xl:items-stretch">
        <div className="flex min-h-0 flex-col gap-5 xl:col-span-4 2xl:col-span-3">
          <LiveStreamControls
        ...
        <div className="flex min-h-[50vh] flex-col xl:col-span-8 xl:min-h-[calc(100dvh-10rem)] 2xl:col-span-9">
          <LiveJobsFeed
